            ...
            switch (changeType) {
              case 'add':
                if (sheets[sheet][cechages].v) sheetResult.push({
                  c: cechages,
                  detail: `${cechages}的值 ${sheets[sheet][cechages].v} 已被添加`
                });
                break;
              case 'delete':
                sheetResult.push({
                  c: cechages,
                  detail: `${cechages}的值已被删除`
                });
                break;
              case 'update':
                // if(cechages === '!merges')
                if (sheets[sheet][cechages].v) sheetResult.push({
                  c: cechages,
                  detail: `${cechages}的值已从 ${workbook1.Sheets[sheet][cechages].v} 更改为 ${sheets[sheet][cechages].v}`
                });
                break;
            }
          });
